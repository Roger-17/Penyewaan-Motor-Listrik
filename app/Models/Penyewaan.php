...
class Penyewaan extends Model
{
    ...
    public static function getListInvoice($id_customer)
    {
        ...
        $sql = "SELECT * FROM penyewaan WHERE id_customer = ? AND status='siap_bayar'";
    ...
    // prosedur input data penyewaan 
    public static function inputPenyewaan($id_customer,$total_harga,$id_barang,$jml_barang,$harga_barang,$total){
        date_default_timezone_set('Asia/Jakarta');
        // query apakah ada di keranjang
        // query kode perusahaan
        $sql = "SELECT COUNT(*) as jml FROM penyewaan WHERE id_customer = ? AND status not in ('expired','selesai','siap_bayar','konfirmasi_bayar')";
        $barang = DB::select($sql,[$id_customer]);
        foreach($barang as $b):
            $jml = $b->jml;
        endforeach;
        ...
        if($jml==0){
            ...
            $sql = "SELECT SUBSTRING(IFNULL(MAX(no_transaksi),'FK-0000'),4)+0 AS no FROM penyewaan";
            $barang = DB::select($sql);
            foreach($barang as $b):
                $urutan = $b->no;
            endforeach;
            ...
            // masukkan ke tabel induk dulu yaitu di tabel penyewaan
            ...
            // masukkan ke tabel detail_penyewaan
            DB::table('penyewaan_detail')->insert([
                'no_transaksi' => $faktur,
                'id_barang' => $id_barang,
            ...
            $sql = "SELECT no_transaksi,MAX(tgl_expired) as mak_expired FROM penyewaan_detail WHERE  
                    no_transaksi IN 
                    (
                        SELECT no_transaksi
                        FROM penyewaan
            ...
            // update ke tabel transaksi expirednya menjadi expired terlama dari detail penyewaan
            $affected = DB::table('penyewaan')
              ->where('no_transaksi', $no_transaksi)
            ...
            if($date>$mak_expired){
                ...
                // kembalikan stok
                $sql = "SELECT id_barang,jml_barang FROM penyewaan_detail WHERE  
                        no_transaksi = ?
                    ";
                ...
                foreach($barang as $b):
                    ...
                    $affected = DB::table('penyewaan_detail')
                ...
                if($cek==0){
                    ...
                    // buat nomor faktur baru dan masukkan ke tabel
                    // dapatkan nomor faktur terakhir cth format FK-0004
                    $sql = "SELECT max(no_transaksi) as no_transaksi  FROM penyewaan
                            WHERE id_customer = ? AND status NOT IN ('selesai','expired','siap_bayar','konfirmasi_bayar')
                           ";
                    $barang = DB::select($sql,[$id_customer]);
                    foreach($barang as $b):
                    ...
                    $sql = "SELECT total_harga  FROM penyewaan
                            WHERE no_transaksi = ? 
                           ";
                    $barang = DB::select($sql,[$no_transaksi]);
                    foreach($barang as $b):
                    ...
                    $date = date('Y-m-d H:i:s');
                    $date_plus_3=Date('Y-m-d H:i:s', strtotime('+3 days')); //tambahkan 3 hari untuk expired datenya
                    // update total harga di penyewaan karena sudah ditambah item baru
                    $affected = DB::table('penyewaan')
                    ->where('no_transaksi', $no_transaksi)
                    ->update(
                                [   'tgl_expired' => $date_plus_3,
    ...
    // untuk menghapus data penyewaan detail
    public static function hapuspenyewaandetail($id_penyewaan_detail){
        $sql = "DELETE FROM penyewaan_detail WHERE id = ?";
        $nrd = DB::delete($sql,[$id_penyewaan_detail]);
    }

    // kembalikan stok
    public static function kembalikanstok($id_penyewaan_detail){

            $sql = "SELECT jml_barang,id_barang FROM penyewaan_detail WHERE id = ?";
            $barang = DB::select($sql,[$id_penyewaan_detail]);
            foreach($barang as $b):
    ...
    public static function getJmlBarang($id_customer){
        $sql = "SELECT count(*) as jml FROM penyewaan_detail WHERE no_transaksi IN (SELECT no_transaksi FROM penyewaan WHERE id_customer = ? AND status NOT IN ('expired','hapus','siap_bayar','konfirmasi_bayar','selesai'))";
        $barang = DB::select($sql,[$id_customer]);
        foreach($barang as $b):
            $jml = $b->jml;
        endforeach;
    ...
    public static function getJmlInvoice($id_customer){
        $sql = "SELECT count(*) as jml FROM penyewaan 
